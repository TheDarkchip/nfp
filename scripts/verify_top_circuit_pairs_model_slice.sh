...
# Canonical circuit-level verification for top GPT-2 small induction heads.
# Emits model slices (LN/QK/OV/unembed) so value/direction checks are anchored to the model.
...
python scripts/build_gpt2_induction_cert.py \
  ...
  --emit-model-slice --model-decimals "${model_decimals}" \

...
@[expose] def toInputAssignment (I : Interface C ι_in ι_out) (input : ι_in → α) :
    C.InputAssignment :=
  fun i => input (I.inputs.symm i)

/-- Evaluate a circuit on a typed interface. -/
@[expose] def eval (I : Interface C ι_in ι_out) (input : ι_in → α) : ι_out → α :=
  fun o => evalInput C (I.toInputAssignment input) (I.outputs o).1

/-- Unfolding equation for `Interface.eval`. -/
private theorem eval_eq (I : Interface C ι_in ι_out) (input : ι_in → α) (o : ι_out) :

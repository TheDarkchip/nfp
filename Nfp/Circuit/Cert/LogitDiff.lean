...
def logitDiffLowerBound (active : Finset (Fin seq))
    ...
    (eps lo hi : Dyadic) (vals : Fin seq → Dyadic) : Option Dyadic := by
  classical
  if h : active.Nonempty then
    let gap := eps * (hi - lo)
    let f : Fin seq → Dyadic := fun q => vals (prev q) - gap
...
def logitDiffLowerBoundAt (active : Finset (Fin seq))
    (prev : Fin seq → Fin seq)
    (epsAt : Fin seq → Dyadic) (lo hi : Dyadic) (vals : Fin seq → Dyadic) : Option Dyadic := by
  classical
  if h : active.Nonempty then
    let gap : Fin seq → Dyadic := fun q => epsAt q * (hi - lo)
    let f : Fin seq → Dyadic := fun q => vals (prev q) - gap q
    let img := active.image f
    have himg : img.Nonempty := h.image f
    exact some (Finset.min' img himg)
  else
...
theorem logitDiffLowerBound_le (active : Finset (Fin seq))
    (prev : Fin seq → Fin seq)
    (eps lo hi : Dyadic) (vals : Fin seq → Dyadic)
...
theorem logitDiffLowerBoundAt_le (active : Finset (Fin seq))
    (prev : Fin seq → Fin seq)
    (epsAt : Fin seq → Dyadic) (lo hi : Dyadic) (vals : Fin seq → Dyadic)
    (q : Fin seq) (hq : q ∈ active) :

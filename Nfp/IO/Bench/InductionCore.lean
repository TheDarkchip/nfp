...
private def isDyadic (q : Dyadic) : Bool :=
  isPow2 q.toRat.den

private def countDyadic {seq dHead : Nat}
    (qs : List (Fin seq)) (ds : List (Fin dHead))
    (f : Fin seq → Fin dHead → Dyadic) : Nat :=
  qs.foldl (fun acc q =>
    ds.foldl (fun acc' d => acc' + (if isDyadic (f q d) then 1 else 0)) acc) 0

private def dyadicSampleReport {seq dModel dHead : Nat}
  ...
  let qLoDy := countDyadic qs ds qkv.qLo
  let qHiDy := countDyadic qs ds qkv.qHi
  let qAbsDy := countDyadic qs ds qkv.qAbs
  let kAbsDy := countDyadic qs ds qkv.kAbs
  let epsDy := if isDyadic inputs.lnEps then 1 else 0
  s!"dyadic sample: total={total} qLo={qLoDy} qHi={qHiDy} qAbs={qAbsDy} " ++
    s!"kAbs={kAbsDy} lnEps={epsDy}"

private def dyadicSanityReport : String :=
  let rat := dyadicOfRatDown (Rat.divInt 1 8)
  let powChecks :=
    s!"pow2(1)={isPow2 1} pow2(2)={isPow2 2} pow2(3)={isPow2 3} " ++
      s!"pow2(4)={isPow2 4} pow2(8)={isPow2 8}"
  let ratCheck := s!"rat(1/8).den={rat.toRat.den} dyadic={isDyadic rat}"
  s!"dyadic sanity: {powChecks} {ratCheck}"

private def forceQRowTasks {seq dHead : Nat}
    (q0 : Fin seq) (qLo : Fin seq → Fin dHead → Dyadic) : Int :=
  let tasks : Array (Task Dyadic) :=
...
private def qAbsRowChunk {seq dHead : Nat}
    (q0 : Fin seq) (qAbs : Fin seq → Fin dHead → Dyadic) (start stop : Nat) : Dyadic :=
...
private def forceQAbsRowTasksReduce {seq dHead : Nat}
    (q0 : Fin seq) (qAbs : Fin seq → Fin dHead → Dyadic) (chunkSize : Nat) : Dyadic :=
  if chunkSize = 0 then
    (0 : Dyadic)
  else
...
private def forceQAbsAllTasksReduce {seq dHead : Nat}
    (qAbs : Fin seq → Fin dHead → Dyadic) (chunkSize : Nat) : Dyadic :=
  let tasks : Array (Task Dyadic) :=
    Array.ofFn (fun q : Fin seq =>
...
private def forceQAbsActiveTasksReduce {seq dHead : Nat}
    (active : Finset (Fin seq)) (qAbs : Fin seq → Fin dHead → Dyadic) (chunkSize : Nat) : Dyadic :=
...
def runCoreBenchFromInputs {seq dModel dHead : Nat} [NeZero seq]
  ...
  let _ ← timePhase "dyadic sample" (do
    IO.println (dyadicSampleReport inputs qkv)
    IO.println dyadicSanityReport
    pure ())

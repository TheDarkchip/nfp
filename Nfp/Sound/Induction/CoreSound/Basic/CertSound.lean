      ...
      · by_cases hmodel : dModel = 0
        ...
        · by_cases hactive : inputs.active.Nonempty
          · let lnBounds := Bounds.cacheBoundPair2 (fun q =>
            ...
            let splitDimsDiffCore : Nat → Fin seq → Fin seq → List (Fin dHead) :=
              fun budget q k =>
              if budget = 0 then
                []
              else
            ...
            let weightBoundAtBaseCached : Fin seq → Fin seq → Rat :=
              certFields.weightBoundAtBaseCached
            ...
            let wvDirRaw : Fin dModel → Rat := fun j =>
              Linear.dotFin dHead dirHead (fun d => inputs.wv j d)
            let wvDirTask : Fin dModel → Rat := Bounds.cacheBoundTask wvDirRaw
            ...
            let valsLo : Fin seq → Rat := fun q =>
              bDir + Bounds.dotIntervalLower wvDir (lnLo q) (lnHi q)
            let valsHi : Fin seq → Rat := fun q =>
              bDir + Bounds.dotIntervalUpper wvDir (lnLo q) (lnHi q)
            let hvalsLo : ∀ k,
                valsLo k = bDir + Bounds.dotIntervalLower wvDir (lnLo k) (lnHi k) := fun _ => rfl
            let hvalsHi : ∀ k,
                valsHi k = bDir + Bounds.dotIntervalUpper wvDir (lnLo k) (lnHi k) := fun _ => rfl
            let univ : Finset (Fin seq) := Finset.univ
            ...
            have hcore' : buildInductionCertFromHeadCoreWith? cfg inputs = some cert := by
              ...
              have hcert_eq :
                  (buildInductionHeadCoreCacheWith cfg inputs).cert = cert := by
                rfl
              simpa [hcert_eq] using hcore''
            have hc : c = cert := by
            ...
            have hscore_bounds :
              ...
              have hscore_base_bounds (hnot : ¬ masked q k) :
                ...
                · have hscale_real : 0 ≤ (inputs.scale : Real) :=
                  ...
                  have hscoreLo : scoreLo q k = inputs.scale * dotLo q k := by
                    simp [scoreLo, masked, hnot, hscale]
                  have hscoreHi : scoreHi q k = inputs.scale * dotHi q k := by
                    simp [scoreHi, masked, hnot, hscale]
                  constructor
                  · simpa [hscoreLo, base, Rat.cast_mul] using hlow
                  · simpa [hscoreHi, base, Rat.cast_mul] using hhigh
                · have hscale_nonpos : inputs.scale ≤ 0 :=
                  ...
                  have hscoreLo : scoreLo q k = inputs.scale * dotHi q k := by
                    simp [scoreLo, masked, hnot, hscale]
                  have hscoreHi : scoreHi q k = inputs.scale * dotLo q k := by
                    simp [scoreHi, masked, hnot, hscale]
                  constructor
                  · simpa [hscoreLo, base, Rat.cast_mul] using hlow
                  · simpa [hscoreHi, base, Rat.cast_mul] using hhigh
            ...
            have hdot_diff_bounds :
              ...
              · by_cases hmem : k ∈ refineKeys q
                · have hlow' :
                    (dotDiffLo q k : Real) ≤
                        dotProduct (fun d => qRealOfInputs inputs q d)
                          (fun d => kRealOfInputs inputs (inputs.prev q) d -
                            kRealOfInputs inputs k d) := by
                    simpa [dotDiffLo, dotDiffLoHi, hbudget, hmem] using hspecRef.1
                  have hhigh' :
                      dotProduct (fun d => qRealOfInputs inputs q d)
                            (fun d => kRealOfInputs inputs (inputs.prev q) d -
                              kRealOfInputs inputs k d) ≤ (dotDiffHi q k : Real) := by
                    simpa [dotDiffHi, dotDiffLoHi, hbudget, hmem] using hspecRef.2
                  exact ⟨hlow', hhigh'⟩
                · have hlow' :
                    (dotDiffLo q k : Real) ≤
                        dotProduct (fun d => qRealOfInputs inputs q d)
                          (fun d => kRealOfInputs inputs (inputs.prev q) d -
                            kRealOfInputs inputs k d) := by
                    simpa [dotDiffLo, dotDiffLoHi, hbudget, hmem] using
                      hspecBase_bounds.1
                  have hhigh' :
                      dotProduct (fun d => qRealOfInputs inputs q d)
                            (fun d => kRealOfInputs inputs (inputs.prev q) d -
                              kRealOfInputs inputs k d) ≤ (dotDiffHi q k : Real) := by
                    simpa [dotDiffHi, dotDiffLoHi, hbudget, hmem] using
                      hspecBase_bounds.2
                  exact ⟨hlow', hhigh'⟩
            have hmarginAt_le :
              ...
              have hmarginAt_eq :
                  marginAt q =
                    (otherKeys q).inf' hnonempty (fun k => scoreGapLo q k) := by
                simp [marginAt, certFields, buildInductionHeadCertFields_def, hq, hnonempty]
              have hle :
            ...
            have hscore_margin_real :
              ...
              have hmargin_le : margin ≤ marginAt q := by
                ...
                simpa [margin, certFields, buildInductionHeadCertFields_def, hnonempty] using hle
            ...
            have hweightBoundAtBase :
              ...
              intro q k hk
              simp [weightBoundAtBase, certFields, buildInductionHeadCertFields_def, hk]
            have hweightBoundAt :
                ∀ q k,
                  weightBoundAt q k = weightBoundAtBase q k := by
              intro q k
              by_cases hk : k = inputs.prev q
              · simp [weightBoundAt, weightBoundAtBase, certFields,
                  buildInductionHeadCertFields_def, Bounds.cacheBound2Task_apply,
                  hk]
              · simp [weightBoundAt, weightBoundAtBase, certFields,
                  buildInductionHeadCertFields_def, Bounds.cacheBound2Task_apply,
                  hk]
            have hepsAt :
              ...
              have hsum :
                  (otherKeys q).sum (fun k =>
                      Bounds.cacheBound2Task
                        (fun q k =>
                          if k = inputs.prev q then
                            (0 : Rat)
                          else if scoreGapLo q k < 0 then
                            (1 : Rat)
                          else
                            ratDivUp 1 (1 + scoreGapLo q k)) q k) =
                    (otherKeys q).sum (fun k =>
                      if scoreGapLo q k < 0 then
                        (1 : Rat)
                      else
                ...
                intro k hk
                have hk' : k ≠ inputs.prev q := by
                  have hk'' : k ∈ Finset.univ.erase (inputs.prev q) := by
                    simpa [otherKeys] using hk
                  exact (Finset.mem_erase.mp hk'').1
                simp [Bounds.cacheBound2Task_apply, hk']
              simpa [epsAt, epsAtBase, certFields, buildInductionHeadCertFields_def, hsum] using
            ...
            have hepsAt_le_eps :
              ...
              have heps_def :
                  eps = inputs.active.sup' hactive epsAt := by
                have heps_def' :
                    certFields.eps =
                      if h : inputs.active.Nonempty then
                        inputs.active.sup' h certFields.epsAt
                      else
                        (0 : Rat) := by
                  simpa [certFields] using
                    (buildInductionHeadCertFields_eps_eq
                      (inputs := inputs) (otherKeys := otherKeys) (scoreGapLo := scoreGapLo))
                have heps_def'' :
                    certFields.eps = inputs.active.sup' hactive certFields.epsAt := by
                  simpa [hactive] using heps_def'
                simpa [eps, epsAt] using heps_def''
              simpa [heps_def] using hle
            ...
            have hsoftmax_bounds :
              ...
              · intro q hq
                ...
                have hle :
                  ...
                  have hle' :=
                    add_le_add_left (hepsAt_le_eps_real q hq) (weights q (inputs.prev q))
                  calc
                    weights q (inputs.prev q) + (epsAt q : Real)
                        = (epsAt q : Real) + weights q (inputs.prev q) := by
                          exact add_comm _ _
                    _ ≤ (eps : Real) + weights q (inputs.prev q) := hle'
                    _ = weights q (inputs.prev q) + (eps : Real) := by
                          exact add_comm _ _
            ...
            have hwvDirRaw :
                ∀ j, wvDirRaw j = Linear.dotFin dHead dirHead (fun d => inputs.wv j d) := by
              intro j
              rfl
            have hwvDirTask : ∀ j, wvDirTask j = wvDirRaw j := by
              intro j
              simpa [wvDirTask] using (Bounds.cacheBoundTask_apply wvDirRaw j)
            have hwvDir : ∀ j, wvDir j = Linear.dotFin dHead dirHead (fun d => inputs.wv j d) := by
              intro j
              have hwv : wvDir j = wvDirTask j := by
                simp only [wvDir, wvDirArr, Array.getElem_ofFn]
              have hwv' : wvDir j = wvDirRaw j := by
                exact hwv.trans (hwvDirTask j)
              calc
                wvDir j = wvDirRaw j := hwv'
                _ = Linear.dotFin dHead dirHead (fun d => inputs.wv j d) := hwvDirRaw j
            have hbDir :
                bDir = Linear.dotFin dHead dirHead (fun d => inputs.bv d) := by
              rfl
            have hvals_bounds :
                ValueIntervalBounds (vals := valsRealOfInputs inputs) valCert :=
              valCert_bounds_of_ln_bounds (inputs := inputs) (dirHead := dirHead) (hdirHead := rfl)
                (wvDir := wvDir) (bDir := bDir) (hwvDir := hwvDir) (hbDir := hbDir)
                (lnLo := lnLo) (lnHi := lnHi) (valsLo := valsLo) (valsHi := valsHi)
                (hvalsLo := hvalsLo) (hvalsHi := hvalsHi) (hln := hln_bounds)
            have hcert_eps : cert.eps = eps := by rfl
            have hcert_margin : cert.margin = margin := by rfl
            have hcert_active : cert.active = inputs.active := by rfl
            have hcert_prev : cert.prev = inputs.prev := by rfl
            have hcert_epsAt : cert.epsAt = epsAt := by rfl
            have hcert_weight : cert.weightBoundAt = weightBoundAt := by rfl
            have hcert_values : cert.values = valCert := by rfl
            refine
              { softmax_bounds := ?_
                oneHot_bounds_at := ?_
                weight_bounds_at := ?_
                value_bounds := ?_ }
            · simpa [hcert_eps, hcert_margin, hcert_active, hcert_prev] using hsoftmax_bounds
            · intro q hq
              simpa [hcert_epsAt, hcert_active, hcert_prev] using oneHot_bounds_at q hq
            · intro q hq k hk
              simpa [hcert_weight, hcert_active, hcert_prev] using weight_bounds_at q hq k hk
            · simpa [hcert_values] using hvals_bounds
          · have : False := by
              have hnone :=
